...
function showLocationAccuracyBanner(detectedCity) {
    ...
    b.innerHTML = '<div style="flex:1">📍 Detectado: <strong>'+detectedCity+'</strong><br><span style="color:#666;font-size:12px">¿No es tu ubicación real? Búscala manualmente.</span></div>'
        + '<button onclick="openSearch();var el=document.getElementById(\'locAccBanner\');if(el)el.remove()" style="background:#4CAF50;color:#fff;border:none;border-radius:8px;padding:6px 12px;font-size:12px;font-weight:700;cursor:pointer;white-space:nowrap">Cambiar</button>'
        + '<button onclick="var el=document.getElementById(\'locAccBanner\');if(el)el.remove()" style="background:none;border:none;color:#999;font-size:16px;cursor:pointer;padding:0 4px">✕</button>';
