...
// Estrategia: 3 proxies en cascada + parseo HTML del catálogo diario
async function fetchCSN(lat, lon) {
    ...
    // URL del catálogo diario CSN (HTML tabla)
    var targetUrl = 'https://www.sismologia.cl/sismicidad/catalogo/'+y+'/'+m+'/'+dateStr+'.html';

    // Proxies en cascada (misma lógica que el resto de sources.js)
    var proxies = [
    ...
    // Columnas: Fecha Local | Lugar | Profundidad | Magnitud
    var alerts = [];
    ...
    rows.forEach(function(row) {
        ...
        var fechaText = (cells[0].textContent || '').trim();
        var lugar     = (cells[1].textContent || '').trim();
        var profText  = (cells[2].textContent || '').trim();
        var magText   = (cells[3].textContent || '').trim();

        if (!fechaText || !magText) return;

        var mag = parseFloat(magText);
        if (isNaN(mag) || mag < 2.0) return; // Solo M2.0+

        // Extraer coordenadas del link de la fila (el CSN linkea cada sismo)
        var link = row.querySelector('a');
        var href = link ? link.getAttribute('href') : '';

        // Intentar extraer lat/lon del href o del texto del lugar
        // href típico: /sismicidad/catalogo/2026/07/20260723030045.html
        // El texto del lugar tiene: "41 km al SE de Constitución"
        var sisoLat = null, sisoLon = null;
        var distMatch = lugar.match(/(\d+)\s*km\s*al\s*(\w+)\s*de\s*(.+)/i);

        if (distMatch) {
            // Buscar ciudad en LocationDatabase o usar coords aproximadas del lugar
            var distKmRef = parseInt(distMatch[1]);
            var direccion = distMatch[2].toUpperCase();
            var ciudadRef = distMatch[3].trim().toLowerCase();

            // Mapa de ciudades chilenas comunes con sus coords
            var coordsCiudades = {
                ...
                'illapel':[-31.63,-71.17],'coquimbo':[-29.95,-71.34],
                'punta arenas':[-53.16,-70.92],'coyhaique':[-45.57,-72.07],
                'puerto natales':[-51.73,-72.50],'cauquenes':[-35.97,-72.32],
                ...
                'vicuna':[-30.03,-70.71],'monte patria':[-30.68,-70.96],
                'salamanca':[-31.77,-70.97],'combarbalá':[-31.18,-71.02],
                'fray jorge':[-30.65,-71.65],'ollagüe':[-21.23,-68.25],
                'ollague':[-21.23,-68.25],'carrizal bajo':[-28.07,-71.14],
                'cañete':[-37.80,-73.40],'lebu':[-37.61,-73.65],
                'arauco':[-37.25,-73.32],'tirua':[-38.35,-73.50],
                'camarones':[-19.02,-69.86],'camina':[-19.57,-69.42]
            };

            // Normalizar nombre ciudad (quitar tildes)
            var ciudadNorm = ciudadRef.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
            var coords = null;
            for (var c in coordsCiudades) {
                var cNorm = c.normalize('NFD').replace(/[\u0300-\u036f]/g,'');
                if (ciudadNorm.includes(cNorm) || cNorm.includes(ciudadNorm)) {
                    coords = coordsCiudades[c]; break;
                }
            }

            if (coords) {
                // Calcular posición aproximada según dirección cardinal
                var kmPerDeg = 111;
                var dLat = 0, dLon = 0;
                if (direccion.includes('N')) dLat =  distKmRef / kmPerDeg;
                if (direccion.includes('S')) dLat = -distKmRef / kmPerDeg;
                if (direccion.includes('E')) dLon =  distKmRef / (kmPerDeg * Math.cos(coords[0]*Math.PI/180));
                if (direccion.includes('O') || direccion.includes('W')) dLon = -(distKmRef / (kmPerDeg * Math.cos(coords[0]*Math.PI/180)));
                // Para diagonales aplicar componente reducida
                if ((direccion.includes('N')||direccion.includes('S')) &&
                    (direccion.includes('E')||direccion.includes('O')||direccion.includes('W'))) {
                    dLat *= 0.707; dLon *= 0.707;
                }
                sisoLat = coords[0] + dLat;
                sisoLon = coords[1] + dLon;
            }
        }
        ...
        // Parsear fecha chilena "2026-07-23 11:04:09"
        var fechaISO = fechaText.replace(' ','T');
        var timeStr;
        try { timeStr = new Date(fechaISO).toLocaleString('es-CL'); }
        catch(e) { timeStr = fechaText; }

        var distKmCalc = (lat && sisoLat) ? Math.round(calcDistance(lat, lon, sisoLat, sisoLon)) : null;

        alerts.push({
            id: 'csn_' + dateStr + '_' + alerts.length,
            type: 'SISMO', icon: icon,
            title: 'Sismo M' + mag.toFixed(1) + ' — ' + lugar,
            description: lugar + ' · Profundidad ' + prof + ' km',
    ...
    console.log('🇨🇱 CSN:', alerts.length, 'sismos hoy');
    return alerts;
}
